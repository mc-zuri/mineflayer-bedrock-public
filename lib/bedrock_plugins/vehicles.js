...
function inject (bot) {
  ...
  // (an angle the delta does not carry is absent, whether or not the protocol also has the flags that say so)
  bot._client.on('move_entity_delta', p => {
    const f = p.flags || {}
    const hasYaw = f.has_rot_y ?? p.rot_y != null
    const hasPitch = f.has_rot_x ?? p.rot_x != null
    if (!hasPitch && !hasYaw) return
    const was = rawRotation.get(String(p.runtime_entity_id)) || { yaw: 0, pitch: 0 }
    trackRotation(p.runtime_entity_id, hasYaw ? byteAngle(p.rot_y) : was.yaw, hasPitch ? byteAngle(p.rot_x) : was.pitch)
  })
  ...
  // bot drives a boat or a horse. `added`: the entity was added just now, with the link held for it.
  function engineVehicle (entity, driver, added = false) {
    ...
    return {
      ...
      // a boat the bot drives starts at rest, as the client takes it over, unless it was added just now (it moves on
      // with the velocity it was added with); one the server moves keeps its velocity
      vel: predicted && !added ? new Vec3(0, 0, 0) : new Vec3(Math.fround(entity.velocity?.x ?? 0), Math.fround(entity.velocity?.y ?? 0), Math.fround(entity.velocity?.z ?? 0)),
  ...
  bot._client.on('set_entity_link', ({ link, held }) => {
    ...
    bot.bedrockVehicle = engineVehicle(vehicle, link.type === LINK_DRIVER, !!held)
