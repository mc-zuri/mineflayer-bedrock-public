...
function inject (bot) {
  ...
  // Layer 0 is the block, layer 1 the liquid a waterlogged block holds.
  bot._client.on('update_block', packet => {
    ...
    if (position) updateBlock(position, packet.block_runtime_id, layer)
  })

  bot._client.on('update_block_synced', packet => {
    if (packet.position) updateBlock(packet.position, packet.block_runtime_id, packet.layer ?? 0)
  })
  ...
  bot._client.on('update_subchunk_blocks', packet => {
    ...
    for (const update of packet.extra || []) updateBlock(update.position, update.runtime_id, 1)
  })

  // The liquid a waterlogged block holds (its second layer), or null when that layer is air or not loaded.
  bot.bedrockLiquidAt = (position) => {
    if (!blockDataSupported) return null
    const point = new Vec3(Math.floor(position.x), Math.floor(position.y), Math.floor(position.z))
    const column = getColumnAt(point)
    if (!column) return null
    const liquid = column.getBlock({ ...localPosition(point), l: 1 }, false)
    return liquid && liquid.name !== 'air' ? liquid : null
  }
  ...
  function updateBlock (position, runtimeId, layer = 0) {
    if (!blockDataSupported) return
    ...
    if (layer !== 0) column.setBlockStateId({ ...localPosition(point), l: layer }, block.stateId)
    else if (world.getColumnAt(point)) world.setBlockStateId(point, block.stateId)
