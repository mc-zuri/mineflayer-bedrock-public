...
function inject (bot) {
  ...
  // A link to an entity not added yet (a vehicle summoned and ridden at once) is held, as the client holds it, and
  // applied when that entity is added.
  const heldLinks = new Map() // unique id -> the set_entity_link packets waiting for it
  function applyHeldLinks (uniqueId) {
    const key = String(uniqueId)
    const held = heldLinks.get(key)
    if (!held) return
    heldLinks.delete(key)
    for (const packet of held) bot._client.emit('set_entity_link', { ...packet, held: true })
  }
  ...
  bot._client.on('set_entity_link', packet => {
    ...
    const missing = !passenger ? link.rider_entity_id : !vehicle ? link.ridden_entity_id : null
    if (missing != null && !removing) {
      const key = String(missing)
      if (!heldLinks.has(key)) heldLinks.set(key, [])
      heldLinks.get(key).push(packet)
      return
    }
    if (!passenger) return
    ...
    if (passenger === bot.entity) {
      ...
      // one the server makes first, the bot leaves as its own (standing at the dismount spot, telling the server where),
      // looking only up the vehicle's column for that spot unless the rider asked to leave
      if (passenger.vehicle) { bot.vehicle = vehicle; bot.emit('mount') } else if (bot.vehicle && typeof bot.dismount === 'function') bot.dismount({ byRider: link.rider_initiated !== false, unlinked: true })
      else if (bot.vehicle) { const was = bot.vehicle; bot.vehicle = null; bot.emit('dismount', was) }
    }
  })
  ...
  bot._client.on('set_entity_data', packet => {
    ...
    // The bot's own glide is the physics' once it runs (the session weighs the server's restatement against it).
    const isGliding = Boolean(entity.metadata.flags?.gliding)
    if (entity !== bot.entity || !bot.bedrockPhysicsState) entity.elytraFlying = isGliding
