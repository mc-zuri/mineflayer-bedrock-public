...
function inject (bot) {
  ...
  bot._client.on('set_entity_link', packet => {
    ...
    if (passenger === bot.entity) {
      ...
      if (passenger.vehicle) { bot.vehicle = vehicle; bot.emit('mount') } else if (bot.vehicle && typeof bot.dismount === 'function') bot.dismount({ byRider: link.rider_initiated !== false })
