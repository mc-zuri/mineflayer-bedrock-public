...
function inject (bot, options = {}) {
  ...
  // A boat just mounted starts from where the client shows it on the first tick ridden (after that tick's
  // interpolation step).
  ...
  bot.on('bedrockDismount', (vehicle, byRider = true, alpha) => {
    ...
    bot.physics.dismount(state, undefined, byRider, alpha)
    state.apply(bot)
    bot.entity.position = state.pos.clone()
    bot.entity.velocity = state.vel.clone()
    written = writtenView()
  ...
  function send (forcedTick) {
    ...
    if (bot._creativeFlying) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        playerState()
        glideRequest()
        // `bot.bedrockTurned === false`: no turn input reached this tick (a replayed recording knows; a rewind then keeps
        // the rotation the tick before ran with)
        session.tick(state, { t, control: state.control, yaw: state.yaw, pitch: state.pitch, turned: bot.bedrockTurned, riptideLaunch: state.riptideLaunch, spinHits: state.spinHits, fireworkUsed: state.fireworkUsed, usingItem: state.usingItem, itemUseStarted: state.itemUseStarted })
        writeBack()
