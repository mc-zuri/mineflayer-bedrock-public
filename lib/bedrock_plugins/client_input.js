...
function inject (bot, options = {}) {
  ...
  function placeRiddenVehicle () {
    ...
    v.pos.set(Math.fround(at.x), Math.fround(at.y), Math.fround(at.z))
    // and turned as it is shown: the seat turns with the vehicle's shown yaw
    if (typeof bot.vehicle.bedrockShownYaw === 'number') v.yaw = bot.vehicle.bedrockShownYaw
  }
  ...
  // The input device the client plays with: 'mouse' (keyboard and mouse), 'game_pad' or 'touch'. It is reported in
  // every packet and changes a few rules (a gamepad descends scaffolding only with its sneak toggle held; touch cancels
  // a sprint when the key lets go). A gamepad's toggle sneak persists the sneak key through an open screen.
  bot.bedrockInputMode = 'mouse'
  bot.bedrockPersistSneak = false
  // The control state the engine reads: the keys, the input device, and whether a window is open (the client clears its
  // input while a screen shows). Anything set on bot.controlState itself wins.
  const controlInput = () => ({
    inputMode: bot.bedrockInputMode,
    persistSneak: bot.bedrockPersistSneak,
    screen: !!bot.currentWindow,
    ...bot.controlState
  })
  ...
  bot.bedrockSession = session
  for (const name of ['move_player', 'change_dimension', 'correct_player_move_prediction', 'set_entity_motion', 'update_attributes', 'set_entity_data', 'mob_effect', 'movement_effect', 'update_player_game_type', 'set_player_game_type', 'set_default_game_type']) {
    bot._client.on(name, packet => session.handlePacket(name, packet))
  }
  // A dimension change holds the player still until the new dimension has loaded: the server has acknowledged the
  // change (its player_action dimension_change_ack) and the column under the player has arrived. A caller that knows
  // when (a replay of a recorded client) sets bot.bedrockDimensionRelease = 'manual' and calls
  // bot.bedrockSession.dimensionLoaded() itself.
  let dimensionAcked = false
  bot._client.on('change_dimension', () => { dimensionAcked = false })
  bot._client.on('player_action', packet => {
    if (packet.action === 'dimension_change_ack' || packet.action === 14) dimensionAcked = true
  })
  function releaseDimension () {
    if (!session.dimensionLoading || bot.bedrockDimensionRelease === 'manual') return
    if (dimensionAcked && state && world.loaded(state.pos)) session.dimensionLoaded()
  }
  ...
  function playerState () {
    const fresh = new PlayerState(bot, controlInput())
  ...
  bot._client.on('respawn', packet => {
    ...
    // nothing the server sent the player that died still applies (a knockback stamped before the death)
    if (typeof session.respawn === 'function') session.respawn()
  ...
  function send (forcedTick) {
    ...
    const reported = stepped ? state : new PlayerState(bot, controlInput())
    ...
    // jumping in a vehicle that does not take the jump, the client leaves it after the tick (unless the caller leaves
    // itself: `bot.bedrockLeaveOnJump` false, as a replay of the client's own leave requests does)
    if (stepped && state.bedrock && state.bedrock.leaveVehicle && bot.bedrockLeaveOnJump !== false) {
