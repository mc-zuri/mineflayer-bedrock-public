...
function inject (bot, options = {}) {
  ...
  function shownPosition (entity) {
    ...
    return state ? state.at : entity.position
  }
  bot.on('entityMoved', entity => {
    if (!SOLID_ENTITIES[entity.name] || !entity.position) return
    const state = interpolation.get(entity.id)
  ...
  // A boat just mounted starts from where the client shows it on the first tick ridden.
  ...
  for (const name of ['move_player', 'correct_player_move_prediction', 'set_entity_motion', 'update_attributes', 'set_entity_data', 'mob_effect', 'movement_effect']) {
  ...
  bot.on('bedrockDismount', (vehicle) => {
    ...
    if (vehicle && state.vehicle.predicted) {
      ...
      interpolation.set(vehicle.id, { at, target: at, steps: 0, entity: vehicle })
      vehicle.bedrockShownPosition = at
    }
    bot.physics.dismount(state)
    state.apply(bot)
    bot.entity.position = state.pos.clone()
    bot.entity.velocity = state.vel.clone()
    written = writtenView()
  ...
  function send (forcedTick) {
    ...
    if (bot._creativeFlying) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        playerState()
        glideRequest()
        session.tick(state, { t, control: state.control, yaw: state.yaw, pitch: state.pitch, riptideLaunch: state.riptideLaunch, spinHits: state.spinHits, fireworkUsed: state.fireworkUsed, usingItem: state.usingItem, itemUseStarted: state.itemUseStarted })
        writeBack()
