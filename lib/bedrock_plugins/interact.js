...
function inject (bot) {
  ...
  // The swing that goes with an attack, a use or a placement
  const swing = (showHand = true) => {
  ...
  // A swing on its own is a swing at nothing: the next tick's input reports it (missed_swing)
  bot.swingArm = (hand = 'right', showHand = true) => {
    if (!bot.entity) return
    swing(showHand)
    bot.missedSwing = true
  }
  ...
  bot.attack = (entity, withSwing = true) => {
